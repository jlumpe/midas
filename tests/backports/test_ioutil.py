...
from midas.backports import ioutil
...
def test_read_write_npy(shape, dtype_str):
	...
	# Write
	ioutil.write_npy(buf, array)
	assert buf.tell() == len(data)

	# Read
	buf.seek(0)
	array2 = ioutil.read_npy(buf, dtype_str, shape)
...
class TestNamedStruct:
	...
	def struct(self, struct_data):
		"""Struct from data."""
		return ioutil.NamedStruct._frombuffer(self.STRUCT_DTYPE, struct_data)
	...
	def field_eq(self, val1, val2):
		...
		if isinstance(val1, ioutil.NamedStruct):
			val1 = val1._astuple()

		if isinstance(val2, ioutil.NamedStruct):
			val2 = val2._astuple()
	...
	def test_eq(self, struct):
		...
		assert struct == ioutil.NamedStruct._frombuffer(self.STRUCT_DTYPE, struct_data)

		# Same dtype, zero data
		assert struct != ioutil.NamedStruct(self.STRUCT_DTYPE)

		# Same data, different dtype
		assert struct != ioutil.NamedStruct._frombuffer(
			[('foo', 'b', struct._size)],
			struct_data,
		)
	...
	def test_read_fields(self, struct):
		...
		for name in struct._names:
			...
			if field_dtype.fields is not None:
				# Sub-struct
				assert isinstance(value, ioutil.NamedStruct)
	...
	def test_write_fields(self, struct):
		...
		for name in struct._names:
			...
			if isinstance(prev_val, ioutil.NamedStruct):
				...
				zero_substruct = ioutil.NamedStruct(prev_substruct._dtype)
