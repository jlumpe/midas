...
Backported from midas.io.signaturefile."""
...
from .ioutil import read_npy, write_npy, NamedStruct
...
class SignatureFile:
	...
	def get_coords_collection(self, indices=None, progress=None, chunksize=None):
		...
		if indices is None:
			...
			# Array to read into
			kcol = KmerCoordsCollection.empty(self.lengths, dtype=self.dtype)
			...
			for begin in range(0, self.count, chunksize):
				...
				kcol.coords_array[kcol.bounds[begin]:kcol.bounds[end]] = values
	...
	def write(cls, fobj, signatures, *, dtype=None, ids=None, metadata=None):
		...
		elif isinstance(signatures, KmerCoordsCollection):
			dtype = signatures.coords_array.dtype
